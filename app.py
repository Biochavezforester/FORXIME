...
# Página de Resultados
    ...
    if st.session_state.results:
        ...
        # Tab 4: Patrones Temporales
            ...
            if results['activity_patterns']:
                ...
                if selected_species:
                    ...
                    fig_activity = visualization.create_activity_pattern_plot(pattern_data)
        ...
        # Tab 7: Mapas de Abundancia
        elif selected_analysis == "🌡️ Mapas de Abundancia":
            st.header("🌡️ Mapas de Abundancia Relativa (RAI)")
            
            st.markdown("""
            Los mapas de abundancia relativa muestran la distribución espacial de las especies mediante 
            interpolación KDE (Kernel Density Estimation). Las zonas de **color rojo** indican alta abundancia, 
            mientras que las **zonas verdes** indican baja abundancia.
            """)
            
            st.info("💡 Selecciona una o más especies para generar mapas de abundancia con interpolación espacial")
            ...
            if st.session_state.wildlife_data is not None:
                ...
                with col1:
                    selected_species_abundance = st.multiselect(
                        "Selecciona especies para mapas de abundancia",
                        options=available_species,
                        default=[],
                        key="abundance_map_species_selector",
                        help="Selecciona una o más especies para generar sus mapas de abundancia"
                    )
                
                with col2:
                    if len(selected_species_abundance) > 0:
                        st.metric("Especies seleccionadas", len(selected_species_abundance))
                
                # Parámetros avanzados
                with st.expander("⚙️ Parámetros Avanzados de Interpolación", expanded=False):
                    col_a, col_b = st.columns(2)
                    
                    with col_a:
                        bandwidth_option = st.selectbox(
                            "Método de Bandwidth",
                            options=["Automático (Scott)", "Personalizado"],
                            index=0,
                            help="El bandwidth controla la suavidad de la interpolación"
                        )
                        
                        if bandwidth_option == "Personalizado":
                            custom_bandwidth = st.slider(
                                "Bandwidth personalizado (metros)",
                                min_value=100,
                                max_value=5000,
                                value=1000,
                                step=100
                            )
                            bandwidth_value = custom_bandwidth
                        else:
                            bandwidth_value = 'auto'
                ...
                if len(selected_species_abundance) > 0:
                    if st.button("🗺️ Generar Mapas de Abundancia", type="primary", key="generate_abundance_maps"):
                        with st.spinner("Generando mapas de abundancia con interpolación KDE..."):
                            from streamlit_folium import folium_static
                            
                            progress_container = st.empty()
                            
                            for idx, species in enumerate(selected_species_abundance):
                                progress_container.text(f"Generando mapa {idx + 1}/{len(selected_species_abundance)}: {species}")
                                
                                # Generar mapa de abundancia
                                abundance_map = visualization.create_abundance_heatmap(
                                ...
                                if abundance_map is not None:
                                    st.subheader(f"📍 {species}")
                                    
                                    # Calcular estadísticas
                                    from modules.spatial_analysis import calculate_rai_by_location, calculate_hotspot_statistics, create_abundance_grid
                                    
                                    rai_data = calculate_rai_by_location(st.session_state.wildlife_data, species)
                                    
                                    if rai_data is not None:
                                        col_m1, col_m2, col_m3, col_m4 = st.columns(4)
                                        col_m1.metric("RAI Promedio", f"{rai_data['RAI'].mean():.2f}")
                                        col_m2.metric("RAI Máximo", f"{rai_data['RAI'].max():.2f}")
                                        col_m3.metric("Cámaras con detección", len(rai_data))
                                        col_m4.metric("Eventos totales", int(rai_data['Eventos'].sum()))
                                    
                                    # Mostrar mapa
                                    folium_static(abundance_map, width=1200, height=600)
                                    
                                    st.markdown("---")
                                else:
                                    st.warning(f"⚠️ No se pudo generar el mapa para {species}. Verifica que haya suficientes datos.")
                            
                            progress_container.empty()
                            st.success(f"✅ Se generaron {len(selected_species_abundance)} mapas de abundancia")
                else:
                    st.warning("⚠️ Selecciona al menos una especie para generar mapas")
                
                # Información adicional
                with st.expander("ℹ️ Acerca de los Mapas de Abundancia"):
                    st.markdown("""
                    ### ¿Qué es el RAI?
                    
                    El **Índice de Abundancia Relativa (RAI)** se calcula como:
                    
                    ```
                    RAI = (Número de eventos independientes / Días-trampa) × 100
                    ```
                    
                    ### Interpolación KDE
                    
                    La **Kernel Density Estimation (KDE)** es un método estadístico que estima la distribución 
                    espacial de abundancia basándose en los puntos de muestreo (cámaras). Crea una superficie 
                    continua que muestra dónde es más probable encontrar la especie.
                    
                    ### Interpretación de Colores
                    
                    - 🟢 **Verde**: Baja abundancia relativa
                    - 🟡 **Amarillo**: Abundancia relativa moderada
                    - 🔴 **Rojo**: Alta abundancia relativa (hotspots)
                    
                    ### Consideraciones
                    
                    - Los mapas son estimaciones basadas en los datos de cámaras trampa
                    - La precisión depende del número y distribución de las cámaras
                    - Los hotspots indican áreas de mayor actividad de la especie
                    """)
            else:
                st.warning("No hay datos disponibles")
